...
import { mySocketId } from '../../connection/socket';
...
const ChessGameWrapper = (props) => {
    ...
    console.log(props.spectator)
    // get the gameId from the URL here and pass it to the chessGame component as a prop. 
    const domainName = 'http://localhost:3000'
    const color = React.useContext(ColorContext)
    const gameid = 'revirtChessGame'
    const [play] = useSound(chessMove);
    const [opponentSocketId, setOpponentSocketId] = React.useState('')
    const [opponentDidJoinTheGame, didJoinGame] = React.useState(props.spectator)
    const [opponentUserName, setUserName] = React.useState('')
    const [gameSessionDoesNotExist, doesntExist] = React.useState(false)
    const [playerColor, setPlayerColor] = React.useState()
    const [disconnection, setDisconnection] = React.useState(false)
    ...
    React.useEffect(() => {
        ...
        //         setOpponentSocketId(statusUpdate.opponentId)
        //     }
        // })
        ...
        socket.on('set opponent', data => {
            ...
            if (socket.id !== data[0].id) {
                opponenet = data[0].userName
                setPlayerColor(false)
            } else {
                setPlayerColor(true)
                opponenet = data[1].userName
            }
            setUserName(opponenet)
            didJoinGame(true)
    ...
    return (
        <React.Fragment>
            {opponentDidJoinTheGame && !disconnection ? (
                <div>
                ...
                <div>
                    <h1 style={{ textAlign: "center", marginTop: "200px" }}>Opponent has left the game</h1>
                </div>
            ) : (
                <div>
                    {/* <h1
